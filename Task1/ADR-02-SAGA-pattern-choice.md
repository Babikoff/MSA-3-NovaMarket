...
![SAGA-оркестрация](seq-order-placement-orchestration.png)

---

### Сравнение хореографии и оркестрации по стандартным критериям
...
**SAGA-оркестрация (асинхронная, поверх Kafka)** — центральный координатор (Order Service) публикует команды (ReserveStock, InitiatePayment, ScheduleDelivery, NotifySeller/Buyer) и слушает события-результаты (StockReserved, PaymentSucceeded, DeliveryScheduled), управляя последовательностью шагов и компенсациями. Альтернатива предпочтительна, если приоритетом являются простота отладки, явный контроль порядка шагов и централизованное управление компенсациями, при этом сохраняя асинхронную развязку через шину событий.
...
**Детализация принятого решения (дополнение к ADR-02, само решение не меняется):**

- [Обзор архитектуры на уровне контейнеров (C2)](C2-architecture-overview.md) — статическая структура системы и шины событий.
- [Сценарии и компенсации SAGA-хореографии](saga-choreography-flow.md) — happy path и все сценарии отказа/отмены с компенсациями.
- [Реестр событий SAGA](saga-events-registry.md) — контракты событий (издатель, тип, имя).
- [Реестр архитектурных решений](adr-index.md) — индекс всех ADR и связанных документов.
